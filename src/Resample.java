 ...
 * All rights reserved. This program (except for ResampleHelper)
...
public class Resample implements JavaSSRC.ProgressListener 
{
		...
		for(i=0;i<argv.length;i++)
		{
			...
			if ("--bits".equals(argv[i])) {
				...
				if (ResamplerHelper.dstBPS != 8 && ResamplerHelper.dstBPS != 16 && ResamplerHelper.dstBPS != 24) {
					System.err.println(String.format("Error: Only 8bit, 16bit and 24bit PCM are supported."));
					return false;
				}
			...
			if ("--isFast".equals(argv[i])) {
				...
				} else if ("standard".equals(argv[i+1])) {
					/* nothing to do */
				} else {
	...
	public Resample(String[] argv){
		...
		if (!ResamplerHelper.quiet) System.err.println(String.format("Shibatch sampling rate converter version %s\n", JavaSSRC.VERSION));
...
/*				if(fType == 3)
			...
			if(!ResamplerHelper.quiet) System.out.println();
			
			if (!ResamplerHelper.twoPass && peak > 1) {
				if (!ResamplerHelper.quiet) System.out.println(String.format("clipping detected : %gdB\n",20*Math.log10(peak)));
			}

			if(!ResamplerHelper.writeDataLengthsToHeader(fpo))
				return;
